...
class iouEval:
    ...
    def fast_hist(self, a, b):
        ...
        return np.bincount(self.nClasses * a[k].astype(int) + b[k],
            minlength=self.nClasses ** 2).reshape(self.nClasses, self.nClasses)
    ...
    def addBatch(self, predict, gth):
        ...
        per_class_acc = np.diag(hist) / (hist.sum(axis=1) + epsilon)
        per_class_iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) -
            np.diag(hist) + epsilon)
