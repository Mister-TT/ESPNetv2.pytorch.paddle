...
class LoadData:
    ...
    def __init__(self, data_dir, classes, cached_data_file, normVal=1.1):
        """
    ...
    def readFile(self, fileName, trainStg=False):
        ...
        with open(self.data_dir + '/' + fileName, 'r') as textFile:
            for line in textFile:
                line_arr = line.split(',')
                img_file = (self.data_dir.strip() + '/' + line_arr[0].strip()
                    ).strip()
                label_file = (self.data_dir.strip() + '/' + line_arr[1].strip()
                    ).strip()
