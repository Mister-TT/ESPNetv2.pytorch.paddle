...
pallete = [[128, 64, 128], [244, 35, 232], [70, 70, 70], [102, 102, 156], [
    ...
    142, 35], [152, 251, 152], [70, 130, 180], [220, 20, 60], [255, 0, 0],
...
def evaluateModel(args, model, image_list):
    ...
    for i, imgName in enumerate(image_list):
        ...
        img_tensor = paddle.to_tensor(data=img)
        img_tensor = paddle.unsqueeze(x=img_tensor, axis=0)
        if args.gpu:
            img_tensor = img_tensor
        img_out = model(img_tensor)
        classMap_numpy = img_out[0].max(0)[1].byte().cpu().data.numpy()
        classMap_numpy = cv2.resize(classMap_numpy, (args.inWidth * 2, args
            .inHeight * 2), interpolation=cv2.INTER_NEAREST)
        if i % 100 == 0 and i > 0:
            print('Processed [{}/{}]'.format(i, len(image_list)))
        name = imgName.split('/')[-1]
        if args.colored:
            classMap_numpy_color = np.zeros((img.shape[1], img.shape[2],
                img.shape[0]), dtype=np.uint8)
            for idx in range(len(pallete)):
                [r, g, b] = pallete[idx]
                classMap_numpy_color[classMap_numpy == idx] = [b, g, r]
            cv2.imwrite(args.savedir + os.sep + 'c_' + name.replace(args.
                img_extn, 'png'), classMap_numpy_color)
            if args.overlay:
                overlayed = cv2.addWeighted(img_orig, 0.5,
                    classMap_numpy_color, 0.5, 0)
                cv2.imwrite(args.savedir + os.sep + 'over_' + name.replace(
                    args.img_extn, 'jpg'), overlayed)
        if args.cityFormat:
            classMap_numpy = relabel(classMap_numpy.astype(np.uint8))
        cv2.imwrite(args.savedir + os.sep + name.replace(args.img_extn,
            'png'), classMap_numpy)
...
def main(args):
    ...
    if not os.path.isfile(args.pretrained):
        ...
        exit(-1)
    modelA = paddle.DataParallel(modelA)
    modelA.set_state_dict(state_dict=paddle.load(path=args.pretrained))
    if args.gpu:
        modelA = modelA
    modelA.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inHeight', type=int, default=512, help=
        'Height of RGB image')
    parser.add_argument('--savedir', default='./results', help=
    ...
    parser.add_argument('--cityFormat', default=True, type=bool, help=
        'If you want to convert to cityscape original label ids')
    parser.add_argument('--colored', default=False, type=bool, help=
        'If you want to visualize the segmentation masks in color')
    parser.add_argument('--overlay', default=False, type=bool, help=
        'If you want to visualize the segmentation masks overlayed on top of RGB image'
        )
    parser.add_argument('--classes', default=20, type=int, help=
